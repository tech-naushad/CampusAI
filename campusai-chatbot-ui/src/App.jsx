...
const App = () => {
  ...
  const handleSend = async (userInput) => {
    ...
    let botMessage = { sender: "assistant", content: "" };
    setMessages((prev) => [...prev, botMessage]);
    ...
    try {
      await BotAPI(
        { message_id: "1", query: userInput, sender: "naushad", timestamp: "12-12-2024" },
        (chunk) => {
          botMessage.content += chunk; // append new chunk
          setMessages((prev) => {
            const updated = [...prev];
            updated[updated.length - 1] = { ...botMessage }; // update last message
            return updated;
          });
        }
      );
      ...
      setMessages((prev) => [
        ...prev.slice(0, -1),
        { sender: "assistant", content: "Sorry, I encountered an error." },
      ]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-100 p-4">
      <div 
      className="flex-1 overflow-y-auto mb-4 p-4 bg-gray-50 rounded-lg shadow-inner">
        {messages.map((msg, index) => (
          <ChatMessage key={index} message={msg} />
        ))}
        {loading && (
          <div className="text-gray-500 text-sm text-center mt-2">Thinking...</div>
        )}
        <div ref={chatEndRef} />
      </div>
